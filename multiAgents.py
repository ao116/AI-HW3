...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState: GameState, action):
        ...
        min_food_distance = 999
        for food in newFood.asList():
            distanceToFood = manhattanDistance(newPos, food)
            if distanceToFood < min_food_distance:
                min_food_distance = float(distanceToFood)
        ...
        if nearestGhostScaredTime > 0:
         min_ghost_distance = -5*min_ghost_distance
        else:
            min_ghost_distance = 5*min_ghost_distance    

        value = successorGameState.getScore() + additionalValue - 10*min_food_distance + min_ghost_distance - 10*numberOfFoodLeft
        return value
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState):
        ...
        def max_agent(state, depth):
            ...
            actions = getLegalActionsNoStop(0, state)
            bestScore = -9999
            score = bestScore
            bestAction=Directions.EAST
            for action in actions:
                score = min_agent(state.generateSuccessor(0, action), depth, 1)
                if score > bestScore:
                    bestScore = score
                    bestAction = action
            if depth == 0:
        ...
        def min_agent(state, depth, ghost):
            ...
            actions = getLegalActionsNoStop(ghost, state)
            leastScore = 99999
            score = leastScore
            for action in actions:
                if next_ghost == 0:
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState):
        ...
        a = -1 * b
        pacman_idx = 0
        actions = gameState.getLegalActions(pacman_idx)
        scores = []

        for action in actions:
            t = getScoreAB(self, gameState.generateSuccessor(pacman_idx, action), 1, 0, a, b)
            scores.append(t)
            a = max(a, t)
...
def getScoreAB(alphaBetaAgent, gameState, idx, depth, a, b):
    ...
    if idx > 0:
        res = sys.maxsize
        for action in moves:
            tScore = getScoreAB(alphaBetaAgent, gameState.generateSuccessor(idx, action), idx + 1, depth, a, b)
            res = min(res, tScore)
            if res < a:
                return res;
            b = min(b, res)
